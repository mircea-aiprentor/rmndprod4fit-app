 ...
 * reels(id, trainerId, title, theme, notes, filename, size, mode, status, video_url, caption,
 ...
 * DEMO: nu urcă nimic (path placeholder). REAL: Supabase Storage (bucket `reels-clipuri`).
 */
...
if (error) throw new Error(`Urcarea fișierului a eșuat: ${error.message}`);
const { data } = supabase.storage.from("reels-clipuri").getPublicUrl(path);
return { storage_path: data.publicUrl, filename: file.name, size: file.size };
}
...
.select().single();
if (error) throw new Error(`Nu am putut crea proiectul: ${error.message}`);
return fromSupabaseReel(data);
}
